...
const getAccountInfo = async () => {
  const response = await axios.get(`${BASE_URL}/account/info?key=${API_KEY}`);
  return response.data;
};

const listFiles = async () => {
  const response = await axios.get(`${BASE_URL}/file/list?key=${API_KEY}`);
  return response.data;
};

const createFolder = async (name) => {
  const response = await axios.get(
    `${BASE_URL}/folder/create?key=${API_KEY}&name=${encodeURIComponent(name)}`
  );
  return response.data;
};
...
const uploadLocalFile = async (filePath, originalName, folderId) => {
